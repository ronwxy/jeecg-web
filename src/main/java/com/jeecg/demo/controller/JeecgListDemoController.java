...
public class JeecgListDemoController extends BaseController {
    ...
    public void minidaoDatagrid(JeecgDemoEntity jeecgDemo, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        String totalSalary = String.valueOf(jeecgMinidaoDao.getSumSalary());
        dataGrid.setFooter("salary:" + ("null".equalsIgnoreCase(totalSalary) ? "0.0" : totalSalary) + ",age,email:合计");
        TagUtil.datagrid(response, dataGrid);
    }
...
//		dataGrid.setFooter("salary,age,name:合计");
    ...
    public void logDatagrid(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        //时间范围查询条件
        String operateTimeBegin = request.getParameter("operatetime_begin");
        String operateTimeEnd = request.getParameter("operatetime_end");
        if (oConvertUtils.isNotEmpty(operateTimeBegin)) {
            try {
                cq.ge("operatetime", DateUtils.parseDate(operateTimeBegin, "yyyy-MM-dd hh:mm:ss"));
            } catch (ParseException e) {
                logger.error(e.toString());
            }
            cq.add();
        }
        if (oConvertUtils.isNotEmpty(operateTimeEnd)) {
            try {
                cq.le("operatetime", DateUtils.parseDate(operateTimeEnd, "yyyy-MM-dd hh:mm:ss"));
            } catch (ParseException e) {
                logger.error(e.toString());
            }
            cq.add();
