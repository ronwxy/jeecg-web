    ...
    public void datagrid(JformOrderCustomer2Entity jformOrderCustomer2, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        if (oConvertUtils.isNotEmpty(mainId)) {
            ...
            try {
                //自定义追加查询条件
                cq.eq("fkId", mainId);
                String queryMoneyBegin = request.getParameter("money_begin");
                String queryMoneyEnd = request.getParameter("money_end");
                if (StringUtil.isNotEmpty(queryMoneyBegin)) {
                    cq.ge("money", Double.parseDouble(queryMoneyBegin));
                }
                if (StringUtil.isNotEmpty(queryMoneyEnd)) {
                    cq.le("money", Double.parseDouble(queryMoneyEnd));
                }
            } catch (Exception e) {
