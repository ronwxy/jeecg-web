...
//				dateFormat, true));
    ...
    public List<?> pageBaseMethod(HttpServletRequest request,
        ...
        int totalPage = 0;
        // 获取当前页
        String strCurrentPage = request.getParameter("str_currentPage");
        currentPage = strCurrentPage == null || "".equals(strCurrentPage) ? 1
                : Integer.parseInt(strCurrentPage);
        // 获取每页的条数
        String strPageRow = request.getParameter("str_pageRow");
        pageRow = strPageRow == null || "".equals(strPageRow) ? pageRow
                : Integer.parseInt(strPageRow);
