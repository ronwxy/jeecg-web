...
public abstract class AbstractGenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
    /**
     * 初始化Log4j的一个实例
     */
    private static final Logger logger = Logger.getLogger(AbstractGenericBaseCommonDao.class);
    /**
...
//                .setProjection(Projections.rowCount())).uniqueResult(), 0);
        return oConvertUtils.getInt(criteria.getExecutableCriteria(getSession()).setProjection(Projections.rowCount()).uniqueResult(), 0);
    }
    ...
    public <T> List<T> findObjForJdbc(String sql, int page, int rows,
        ...
        for (Map<String, Object> m : mapList) {
            try {
                ...
                MyBeanUtils.copyMap2BeanNobig(po, m);
                rsList.add(po);
            } catch (Exception e) {
                e.printStackTrace();
            }
