 ...
 */
@Repository
public class CommonDao extends AbstractGenericBaseCommonDao implements ICommonDao, IGenericBaseCommonDao {
        ...
        if (uploadFile.getFileKey() != null) {
            ...
            try {
                ...
                if (uploadFile.getCusPath() != null) {
                    ...
                    realPath += DateUtils.getDataString(DateUtils.YYYY_MM_DD) + "/";
                    path += DateUtils.getDataString(DateUtils.YYYY_MM_DD) + "/";
                    file = new File(realPath);
                    if (!file.exists()) {
                ...
                for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
                    ...
                    if (uploadFile.isRename()) {
                        ...
                        noextfilename = DateUtils.getDataString(DateUtils.YYYY_MM_DD_HH_MM_SS) + StringUtil.random(8);//自定义文件名称
...
//				FileCopyUtils.copy(mf.getBytes(), savefile);
                    ...
                    for (int k = 0; k < fields.length; k++) {
                        if (node.getName().equals(fields[k].getName())) {
                            ...
                            if ("int".equals(type)) {
                                ...
                                setMethod.invoke(obj1, new Timestamp(DateUtils.str2Date(node.getText(), DateUtils.DATETIME_FORMAT).getTime()));
                            }
                        }
                    }
                }
    ...
    public Long getAggregateResult(final DetachedCriteria dc){
            Criteria criteria = dc.getExecutableCriteria(
                    getSession());
