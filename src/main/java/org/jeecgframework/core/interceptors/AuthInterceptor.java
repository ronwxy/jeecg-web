    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
        ...
        if (excludeUrls.contains(requestPath)) {
            ...
            if (currLoginUser != null) {
                ...
                } else if ("cgAutoListController.do?list".equals(requestPath)) {
                ...
                if (!"admin".equals(loginUserName)) {
                    ...
                    List<TSDataRule> menuDataAuthorRules = new ArrayList<>();
                    String menuDataAuthorRuleSql = "";
                    ...
                    for (String dataRuleId : dataRuleIds) {
                        ...
                        menuDataAuthorRules.add(dataRule);
                        menuDataAuthorRuleSql += SysContextSqlConvert.setSqlModel(dataRule);
                    }
                    //【加载数据权限】数据权限规则，Hibernate字段方式
                    JeecgDataAutorUtils.installDataSearchConditon(request, menuDataAuthorRules);
                    //【加载数据权限】数据权限规则，Sql方式
                    JeecgDataAutorUtils.installDataSearchConditon(request, menuDataAuthorRuleSql);
                }
                return true;
            } else {
