...
public class RedisTokenManager implements TokenManager {
     ...
     */
    @Override
    public String createToken(TSUser user,String robotId) {
        //使用uuid作为源token
        //按分钟变动 幼儿园token失效后重登问题解决
        Calendar cal = DateUtils.getCalendar();
        cal.setTime(new Date());
        cal.set(Calendar.SECOND, 0);
        cal.set(Calendar.MILLISECOND, 0);
        String token = Jwts.builder().setId(user.getUserName()).setSubject(robotId).setIssuedAt(cal.getTime()).signWith(SignatureAlgorithm.HS256, JwtConstants.JWT_SECRET).compact();
        //存储到redis并设置过期时间
        redisTemplate.boundValueOps(user.getUserName()+"."+robotId).set(token, JwtConstants.TOKEN_EXPIRES_HOUR, TimeUnit.HOURS);
        return token;
    }
