...
import org.jeecgframework.tag.core.factory.AbstractComponentFactory;
    ...
    public void setColumn(String title, String field, Integer width, Integer showLen, String rowspan,
        ...
        if (StringUtil.isNotEmpty(replace)) {
            ...
            for (String string : test) {
                langKey = string.substring(0, string.indexOf("_"));
                text += MutiLangUtil.getLang(langKey) + ",";
...
//        log.info("================================ DataGridTag 开始时间:"+sdf.format(new Date())+"==============================");
        ...
        try {
            ...
            if ("bootstrap-table".equals(component)) {
                AbstractComponentFactory componentFactory = new BootstrapTableComponent();
                ...
                if (oConvertUtils.isNotEmpty(column.getReplace())) {
                    ...
                    if (replaceArray.length > 0) {
                        ...
                        for (String replaceOri : replaceArray) {
                            String langKey = replaceOri.split("_")[0];
                            text = MutiLangUtil.getLang(langKey);
                ...
                if (column.isQuery()) {
                    ...
                    if (oConvertUtils.isNotEmpty(dictionary)) {
                        ...
                        if (showMode != null && "radio".equals(showMode)) {
                            ...
                            String value = "";
                            for (String string : test) {
                                String langKey = string.split("_")[0];
                                text = MutiLangUtil.getLang(langKey);
...
//					}
        ...
        if ("single".equals(getQueryMode()) && hasQueryColum(columnList)) {//如果表单是单查询
            ...
            sb.append("<span style=\"float:right\">");
            if (btnCls != null && !"easyui".equals(btnCls)) {// 自定以样式 bootstrap按钮样式
        ...
        if (superQuery) {
            if (btnCls != null && !"easyui".equals(btnCls)) {//自定义bootstrap按钮样式
    ...
    private void loadToolbarMoreBtn(StringBuffer sb, boolean isShow, DataGridUrl toolBar) {
        if (isShow) {
            if (btnCls != null && !"easyui".equals(btnCls)) {
                if (btnCls.indexOf("bootstrap") == 0) {
                    if (btnCls.replace("bootstrap", "").trim().length() > 0) {
            ...
            } else if (btnCls == null || "easyui".equals(btnCls)) {
    ...
    private void loadToolbar(DataGridUrl toolBar, StringBuffer sb) {
        ...
        } else if (btnCls == null || "easyui".equals(btnCls)) {//easyUI按钮样式
...
//	                                    + " onMouseOver=\"toolTip('<img src='+value+'>')\" onMouseOut=\"toolTip()\" "
                    ...
                    } else if (StringUtil.isNotEmpty(column.getField()) && "opt".equals(column.getField())) {// 加入操作
                        ...
                        sb.append(column.getShowLen());
                        sb.append(")+'...';}}");
                    } else if (columnValueList.size() > 0 && StringUtil.isNotEmpty(column.getField()) && !"opt".equals(column.getField())) {// 值替換
                ...
                if (columnStyleList.size() > 0 && StringUtil.isNotEmpty(column.getField()) && !"opt".equals(column.getField())) {
    ...
    public String getNoAuthOperButton() {
        StringBuffer sb = new StringBuffer();
        if ("admin".equals(ResourceUtil.getSessionUser().getUserName()) || !Globals.BUTTON_AUTHORITY_CHECK) {
        } else {
            Set<String> operationCodes = (Set<String>) super.pageContext.getRequest().getAttribute(Globals.OPERATIONCODES);
            if (null != operationCodes) {
                for (String myOperationCode : operationCodes) {
...
//		appendLine(sb,"<script type=\"text/javascript\" src=\"plug-in/jquery-plugs/storage/jquery.storageapi.min.js\" ></script>");
    ...
    private void addSuperQuery(StringBuffer sb, String buttonSytle, List<DataGridColumn> columnList) {
        ...
        Map<String, Object> mainConfig = new HashMap<>(10);
    ...
    private boolean checkBrowerIsNotIE() {
        String browserType = "";
        Object browerType = ContextHolderUtils.getSession().getAttribute("brower_type");
        if (browerType == null) {
            ...
            browserType = browerType.toString();
        }
    ...
    public boolean hasGroup() {
        boolean flag = false;
        for (DataGridColumn column : columnList) {
            if (StringUtil.isNotEmpty(column.getField()) && "opt".equals(column.getField())) {
                if (column.isOptsMenu()) {
                    return false;
                }
            }
