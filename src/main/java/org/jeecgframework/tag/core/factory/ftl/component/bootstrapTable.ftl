                        ...
                        <#-- update-end-author:taoYan date:20180622 for:TASK #2815 【待完善功能】datagrid 设为bootstrap 风格时， superQuery="true" 时，高级查询按钮不显示 -->
                            ...
                            <div class="col-xs-12 col-sm-6 col-md-4">
                                <div class="input-group col-md-12" style="margin-top:20px">
                                    <a type="button" onclick="searchList();"
                                       class="btn btn-primary btn-rounded  btn-bordered btn-sm"><span
                                            class="glyphicon glyphicon-search" aria-hidden="true"></span> 查询</a>
                                    <a type="button" onclick="searchRest();"
                                       class="btn btn-primary btn-rounded  btn-bordered btn-sm"><span
                                            class="glyphicon glyphicon-repeat" aria-hidden="true"></span> 重置</a>
                                <#-- update-begin-author:taoYan date:20180622 for:TASK #2815 【待完善功能】datagrid 设为bootstrap 风格时， superQuery="true" 时，高级查询按钮不显示 -->
			                         <#if dataGrid.superQuery == true>
			                         	<a type="button" onclick="bootstrapQueryBuilder()"
                                           class="btn btn-primary btn-rounded  btn-bordered btn-sm"><span
                                                class="glyphicon glyphicon-repeat" aria-hidden="true"></span>高级查询</a>
                                     </#if>
                                <#-- update-end-author:taoYan date:20180622 for:TASK #2815 【待完善功能】datagrid 设为bootstrap 风格时， superQuery="true" 时，高级查询按钮不显示 -->
                                </div>
                            </div>
...
<div class="panel-body" style="padding-top:0px;padding-bottom:0px;">
    ...
    <div id="toolbar">
            ...
            <#-- update-begin-author:liushaoqian date:201800713 for:TASK #2962 【online表单开发--刘少谦】 测试问题 -->
					<button
                            onclick="${po.funname}('${po.title}','${po.url}','${dataGrid.name}','${po.width}','${po.height}')"
                    <#-- update-begin-author:liushaoqian date:201800713 for:TASK #2962 【online表单开发--刘少谦】 测试问题 -->
					<#if po.icon=="icon-add">
...
<script type="text/javascript">
    ...
    var TableInit = function () {
        ...
        oTableInit.Init = function () {
            $('#${dataGrid.name}').bootstrapTable({
                ...
                <#if dataGrid.getSingleSelect()==true>
                singleSelect: true,                // 单选checkbox
                </#if>
            <#--<#if dataGrid.pagination == true>  //##################################################需要控制是否分页，会报错-->
                pagination: true,                   //是否显示分页（*）
                pageNumber: 1,                       //初始化加载第一页，默认第一页
                pageSize: ${dataGrid.getPageSize()},                       //每页的记录行数（*）
            ...
            <#-- update-end-author:jiaqiankun date:20180704 for:TASK #2882 【bootstrapTable】代码生成器存在的问题 bootstrap表格排序 -->
                ...
                <#if dataGrid.getHeight() == null>
                height: $(window).height() - 35,   //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
                <#else>
                height: ${dataGrid.getHeight()},   //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
                </#if>
                ...
                <#-- update-end-author:zhoujf date:20180712 for:TASK #2809 【bug】网友反馈问题，字典性能问题 -->
                    console.info("加载成功");
                    <#if dataGrid.getOnLoadSuccess()??>
                    ${dataGrid.getOnLoadSuccess()}();
                    </#if>
                },
    ...
    function reloadTable(newUrl) {
        var opt = {
            url: newUrl,
            silent: true
        };
        $('#${dataGrid.name}').bootstrapTable('refresh', opt);
    }
    ...
    var get${dataGrid.name}Selections = function (field) {
        var ids = [];
        var rows = $('#${dataGrid.name}').bootstrapTable('getSelections');
