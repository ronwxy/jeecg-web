...
public class CgformFtlController extends BaseController {
    ...
    public ModelAndView addorupdate(CgformFtlEntity cgformFtl,
        ...
        sb.append(" function del(url,obj){$.dialog.confirm(\"确认删除该条记录?\", function(){$.ajax({async : false,cache : false,type : 'POST',url : url,error : function() {},success : function(data) {var d = data; if (d.success) {var msg = d.msg;tip(msg);$(obj).closest(\"tr\").hide(\"slow\");}}});}, function(){ });}");
        sb.append("</script>");
    ...
    public AjaxJson saveWordFiles(HttpServletRequest request,
        ...
        try {
            ...
            if (uploadFile.getCusPath() != null) {
                ...
                realPath += DateUtils.getDataString(DateUtils.YYYY_MM_DD) + File.separator;
                path += DateUtils.getDataString(DateUtils.YYYY_MM_DD) + File.separator;
                file = new File(realPath);
                if (!file.exists()) {
            ...
            for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
                ...
                if (uploadFile.isRename()) {
                    ...
                    noextfilename = DateUtils
                            .getDataString(DateUtils.YYYY_MM_DD_HH_MM_SS)
                ...
                script.append(" function del(url,obj){$.dialog.confirm(\"确认删除该条记录?\", function(){$.ajax({async : false,cache : false,type : 'POST',url : url,error : function() {},success : function(data) {var d = data;if (d.success) {var msg = d.msg;tip(msg);$(obj).closest(\"tr\").hide(\"slow\");}}});}, function(){ });}");
