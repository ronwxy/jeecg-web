...
public class TempletContextWord {
    ...
    static {
        ...
        sysMode = util.readProperty(CgAutoListConstant.SYS_MODE_KEY);
    }
...
//        }
        ...
        try {
//			if(CgAutoListConstant.SYS_MODE_DEV.equalsIgnoreCase(sysMode)){//开发模式
            template = freemarker.getTemplate(tableName, freemarker.getLocale(), ENCODING);
//			}else if(CgAutoListConstant.SYS_MODE_PUB.equalsIgnoreCase(sysMode)){//生产模式（缓存）
//				//获取版本号
...
//		html = html.replace("<html xmlns:m=\"http://schemas.microsoft.com/office/2004/12/omml\">", "<%@ page language=\"java\" import=\"java.util.*\" contentType=\"text/html; charset=UTF-8\" pageEncoding=\"UTF-8\"%><br><%@include file=\"/WEB-INF/jsp/context/mytags.jsp\"%>");
