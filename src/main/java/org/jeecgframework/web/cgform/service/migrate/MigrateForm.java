...
public class MigrateForm<T> {
    ...
    public static List<String> createSQL(String ids, JdbcTemplate jdbcTemplate) {
        ...
        for (String id : idList) {
            lsSql = "select * from cgform_head where id='" + id + "'"; // 获得导出表单
            listSQL.add(lsSql);

            lsTmpSql = "select * from cgform_field where table_id='" + id + "'"; // 获得导出表单的字段
            listSQL.add(lsTmpSql);
            // 获得自定义按钮数据
            lsTmpSql = "select * from cgform_button where form_id ='" + id + "'";
            listSQL.add(lsTmpSql);
            // 获得JS增强数据
            lsTmpSql = "select * from cgform_enhance_js where form_id ='" + id + "'";
            listSQL.add(lsTmpSql);
            // 获得SQL增强数据
            lsTmpSql = "select * from cgform_button_sql where form_id ='" + id + "'";
            listSQL.add(lsTmpSql);
            // 获得模板数据
            lsTmpSql = "select * from cgform_ftl where cgform_id ='" + id + "'";
            listSQL.add(lsTmpSql);
            // 获得上传文件数据
            lsTmpSql = "select * from cgform_uploadfiles where cgform_id ='" + id + "'";
            listSQL.add(lsTmpSql);

            rowsList = jdbcTemplate.queryForList(lsSql);
            if (rowsList != null && rowsList.size() > 0) {
                ...
                if (subTable != null && !"".equals(subTable)) {
                    ...
                    for (String sub : subs) {
                        // 获得导出子表表单
                        lsTmpSql = "select * from cgform_head where table_name='" + sub + "'";
                        listSQL.add(lsTmpSql);
                        subRowsList = jdbcTemplate.queryForList(lsTmpSql);
                        if (subRowsList != null && subRowsList.size() > 0) {
                            subSqlMap = (Map) subRowsList.get(0);
                            lsSubId = (String) subSqlMap.get("id");
                            // 获得导出子表字段
                            lsTmpSql = "select * from cgform_field where table_id='" + lsSubId + "'";
                            listSQL.add(lsTmpSql);
                            // 获得子表自定义按钮数据
                            lsTmpSql = "select * from cgform_button where form_id ='" + lsSubId + "'";
                            listSQL.add(lsTmpSql);
                            // 获得子表JS增强数据
                            lsTmpSql = "select * from cgform_enhance_js where form_id ='" + lsSubId + "'";
                            listSQL.add(lsTmpSql);
                            // 获得子表SQL增强数据
                            lsTmpSql = "select * from cgform_button_sql where form_id ='" + lsSubId + "'";
                            listSQL.add(lsTmpSql);
                            // 获得子表模板数据
                            lsTmpSql = "select * from cgform_ftl where cgform_id ='" + lsSubId + "'";
                            listSQL.add(lsTmpSql);
                            // 获得子表上传文件数据
                            lsTmpSql = "select * from cgform_uploadfiles where cgform_id ='" + lsSubId + "'";
                            listSQL.add(lsTmpSql);
                        }
                    }
                }
    ...
    public static List<DBTable> buildExportDbTableList(String ids, JdbcTemplate jdbcTemplate) throws Exception {
        ...
        for (String id : idList) {
            lsSql = "select * from cgform_head where id='" + id + "'"; // 获得导出表单
            listTables.add(bulidDbTableFromSQL(lsSql, CgFormHeadPojo.class, jdbcTemplate));

            lsTmpSql = "select * from cgform_index where table_id='" + id + "'"; // 获得导出索引的字段
            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgFormIndexPojo.class, jdbcTemplate));


            lsTmpSql = "select * from cgform_field where table_id='" + id + "'"; // 获得导出表单的字段
            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgFormFieldPojo.class, jdbcTemplate));
            // 获得自定义按钮数据
            lsTmpSql = "select * from cgform_button where form_id ='" + id + "'";
            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgformButtonEntity.class, jdbcTemplate));
            // 获得JS增强数据
            lsTmpSql = "select * from cgform_enhance_js where form_id ='" + id + "'";
            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgformEnhanceJsEntity.class, jdbcTemplate));
            // 获得SQL增强数据
            lsTmpSql = "select * from cgform_button_sql where form_id ='" + id + "'";
            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgformButtonSqlEntity.class, jdbcTemplate));
            // 获得模板数据
            lsTmpSql = "select * from cgform_ftl where cgform_id ='" + id + "'";
            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgformFtlEntity.class, jdbcTemplate));
            // 获得上传文件数据
            lsTmpSql = "select * from cgform_uploadfiles where cgform_id ='" + id + "'";
            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgUploadEntity.class, jdbcTemplate));
            ...
            if (rowsList != null && rowsList.size() > 0) {
                ...
                if (subTable != null && !"".equals(subTable)) {
                    ...
                    for (String sub : subs) {
                        // 获得导出子表表单
                        lsTmpSql = "select * from cgform_head where table_name='" + sub + "'";
                        listTables.add(bulidDbTableFromSQL(lsTmpSql, CgFormHeadPojo.class, jdbcTemplate));
                        subRowsList = jdbcTemplate.queryForList(lsTmpSql);
                        if (subRowsList != null && subRowsList.size() > 0) {
                            subSqlMap = (Map) subRowsList.get(0);
                            lsSubId = (String) subSqlMap.get("id");
                            ...
                            // 获得导出子表索引
                            lsTmpSql = "select * from cgform_index where table_id='" + lsSubId + "'";
                            listTables.add(bulidDbTableFromSQL(lsTmpSql, CgFormIndexPojo.class, jdbcTemplate));
                    ...
                    for (int i = 1; i <= columnCount; i++) {
                        ...
                        if (i == 1) {
                            ...
                            if (Types.CHAR == sqlRsmd.getColumnType(i) || Types.VARCHAR == sqlRsmd.getColumnType(i) || Types.LONGVARCHAR == sqlRsmd.getColumnType(i)) {
                                ...
                                String lsTmp = getBlob(lsId, tableName, sqlRsmd.getColumnName(i), jdbcTemplate);
                                columnValue.append(lsTmp).append(",");
                            } else {
                                columnValue.append(value).append(",");
                            }
                        }
    ...
    public static String getBlob(String id, String tableName, final String columnName, JdbcTemplate jdbcTemplate) {
        String lsSql = "select " + columnName + " from " + tableName + " where id='" + id + "'";

        // 查询并获得输入流
        jdbcTemplate.query(lsSql, new RowCallbackHandler() {
        ...
        if (inStream != null) {
            ...
            String lsReturn = readInBuffer.toString().trim();
            if ("0x".equals(lsReturn)) {
                lsReturn = lsReturn + "00";
            }
            return lsReturn;
        } else {
            return "0x00";
