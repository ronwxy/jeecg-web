...
//                templateName=entity.getTemplateDetailName();
    ...
    private static String getListCtrl(JSONObject item,
        ...
        tempHtml += "$(function(){";
        tempHtml += "$(\"#listAdd\").click(function(){";
        tempHtml += "var tbHtml ='<tr>'+ $(\"#" + tableNm + " tr\").eq(2).html().replace('<td></td>',\"<td><a href='javascript:void(0);' class='delrow'>删除</a></td>\")+'</tr>';";
        //因为获取的第一行没有删除功能，需加上
        if (isExistSum) {//存在合计行，则将行插入到倒数第二行的后面
            tempHtml += "$(\"#" + tableNm + " tr\").eq(-2).after(tbHtml);";
        } else {//不存在合计行，则将行插入到最后一行
            tempHtml += "$(\"#" + tableNm + " tr:last\").after(tbHtml);";
        }
        tempHtml += "$(\".delrow\").die().live(\"click\",function(){$(this).parent().parent().remove();});";
        tempHtml += "});";
        //添加删除按钮的方法
        tempHtml += "$(\".delrow\").click(function(){$(this).parent().parent().remove();});";
        tempHtml += "});</script>";
