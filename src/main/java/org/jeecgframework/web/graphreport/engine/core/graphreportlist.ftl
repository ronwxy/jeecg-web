...
<div class="operations" style="text-align: right;border-top:1px solid #e7eaec;">
    <div class="bd3">
		<#list config_queryList as x>
			...
			<span style="display:-moz-inline-box;display:inline-block;width: 100px;text-align:right;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; position: relative;top: 3px"
                  title="${x['field_txt']}">${x['field_txt']}：</span>
			<#if x['search_mode']=="group">
				<input type="text" name="${x['field_name']}_begin" style="width: 94px"
                       <#if x['field_type']=="Date">class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"</#if> />
			...
			<#if x['search_mode']=="single">
                <#if  (x['field_dictlist']?size >0)>
                    ...
                    <#else>
						<select name="${x['field_name']}"
                                style="width: 104px" <#if  (x['field_dictlist']?size >10)></#if>>
						<#if  (x['field_dictlist']?size <= 10)>
		...
		<#if (config_queryList?size > 0)>
			<span style="display:-moz-inline-box;display:inline-block;margin-left: 20px;">
				<a href="#" id="searchBtn"><img style="top: 3px;position: relative;"
                                                src="plug-in/graphreport/css/images/search.png"> 查询</a> &nbsp;
				<a href="#" id="resetBtn"><img style="top: 3px;position: relative;"
                                               src="plug-in/graphreport/css/images/reset.png"> 重置</a>
			</span>
        </#if>
    </div>
</div>
...
<script>
    ...
    <#else>
		var categories = $.map(data, function (n) {
            return n["${main['categories']}"]
        });
    </#if>
        //数据转换成报表格式
        ...
        <#if (tabList?size > 1)>
			if (tabName || $.trim(tabName)) {
                if ("${x['tab_name']!''}" == tabName) {
                    series.push({
                        type: "${x['graph_type']}", name: "${x['graph_name']}", data: $.map(data, function (n) {
...
var columns = new Array();
    <#list config_fieldList as x>
		...
		columns.push({
            field: "${x['field_name']}",
            title: "<#if x['field_txt']??>${x['field_txt']}</#if>",
            width: 120,
            sortable: true
        });
        </#if>
