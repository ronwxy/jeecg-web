...
public class LoginController extends BaseController {
    ...
    private static final String FINEUI = "fineui";
    private static final String ACE = "ace";
    private static final String DIY = "diy";
    private static final String ACELE = "acele";
    private static final String HPLUS = "hplus";
    private static final int MS_S = 1000;
    private static final int S_M_H = 60;
    private static final int THREE_HOUR = 3;
    private static final int FIVE_MINUTE = 3;
    ...
    public AjaxJson checkuser(TSUser user, HttpServletRequest req) {
        ...
        //验证码
        String randCodeStr = "randCode";
        String randCode = req.getParameter(randCodeStr);
        if (StringUtils.isEmpty(randCode)) {
            j.setMsg(mutiLangService.getLang("common.enter.verifycode"));
            j.setSuccess(false);
        } else if (!randCode.equalsIgnoreCase(String.valueOf(session.getAttribute(randCodeStr)))) {
            ...
            if (u != null && u.getStatus() != 0) {
                ...
                String interfaceUser = "2";
                if (interfaceUser.equals(u.getUserType())) {
    ...
    public String login(ModelMap modelMap, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (user != null) {
            ...
            modelMap.put("roleName", roles.length() > 15 ? roles.substring(0, 15) + "..." : roles);
            modelMap.put("realName", user.getRealName().length() > 8 ? user.getRealName().substring(0, 8) + "..." : user.getRealName());
            modelMap.put("userName", user.getUserName().length() > 8 ? user.getUserName().substring(0, 8) + "..." : user.getUserName());
            ...
            if (FINEUI.equals(sysTheme.getStyle()) || ACE.
                    equals(sysTheme.getStyle()) || DIY.equals(sysTheme.getStyle()) || ACELE.equals(sysTheme.getStyle()) || HPLUS.equals(sysTheme.getStyle())) {
                request.setAttribute("menuMap", userService.getFunctionMap(user.getId()));
            }
            //国际化cookie
            Cookie i18NCookie = new Cookie("i18n_browser_Lang", oConvertUtils.getString(request.getSession().getAttribute("lang")));
            //设置cookie有效期为一个月
            i18NCookie.setMaxAge(3600 * 24 * 30);
    ...
    public AjaxJson resetPwd(String key, String password) {
        ...
        Date now = new Date();
        if (passwordResetkey != null && passwordResetkey.getIsReset() != 1 && (now.getTime() - passwordResetkey.getCreateDate().getTime()) < MS_S * S_M_H * S_M_H * THREE_HOUR) {
    ...
    public AjaxJson sendResetPwdMail(String email, HttpServletRequest request) {
        ...
        try {
            ...
            String hql = "from TSPasswordResetkey bean where bean.username = ?0 and bean.isReset = 0 order by bean.createDate desc limit 1";
            ...
            if (resetKeyList != null && !resetKeyList.isEmpty()) {
                ...
                Date now = new Date();
                if (resetKey.getEmail().equals(email) && (now.getTime() - resetKey.getCreateDate().getTime()) < (MS_S * S_M_H * S_M_H * THREE_HOUR - MS_S * S_M_H * FIVE_MINUTE)) {
                    ajaxJson.setSuccess(false);
            ...
            if (content.indexOf(userNameStr) > -1) {
            ...
            //配置邮件模板参数
            Map<String, Object> mailConfig = new HashMap<>(4);
            ...
            String authenticationFailedException = "javax.mail.AuthenticationFailedException";
            if (authenticationFailedException.equals(e.getClass().getName())) {
                ajaxJson.setSuccess(false);
    ...
    public ModelAndView shortcutTop(HttpServletRequest request) {
        TSUser user = ResourceUtil.getSessionUser();
        HttpSession session = ContextHolderUtils.getSession();
        // 登陆者的权限
        if (user.getId() == null) {
