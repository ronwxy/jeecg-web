...
public class RoleController extends BaseController {
    ...
    public AjaxJson delUserRole(@RequestParam(required = true) String userid, @RequestParam(required = true) String roleid) {
        ...
        try {
            ...
            if (roleUserList.size() == 1) {
                ...
                String sql = "delete from t_s_role_user where userid = ?0 and roleid = ?1";
    ...
    public AjaxJson delRole(TSRole role, HttpServletRequest request) {
        ...
        if (count == 0) {
            ...
            systemService.executeSql("delete from t_s_role_org where role_id=?0", role.getId()); // 删除 角色-机构 关系信息
    ...
    public List<ComboTree> getRoleTree(HttpServletRequest request) {
        ...
        List<TSRole[]> orgRoleArrList = systemService
                .findHql(
                        "from TSRole r, TSRoleOrg ro, TSDepart o WHERE r.id=ro.tsRole.id AND ro.tsDepart.id=o.id AND o.id=?0",
                        orgId);
    ...
    public AjaxJson updateOrgRole(HttpServletRequest request) {
        ...
        try {
            ...
            systemService.executeSql("delete from t_s_role_org where org_id=?0",
                    orgId);
