...
//			depart.setOrgCode(orgCode);
    ...
    private synchronized String getMaxLocalCode(String parentCode) {
        ...
        int localCodeLength = parentCode.length() + YouBianCodeUtil.ZHAN_WEI_LENGTH;
    ...
    public AjaxJson saveOperate(HttpServletRequest request) {
        ...
        // 录入操作前清空上一次的操作数据
        clearp(roleid);
        String[] funOp = fop.split(",");
        String aa = "";
        String bb = "";
        // 只有一个被选中
        if (funOp.length == 1) {
            bb = funOp[0].split("_")[1];
            aa = funOp[0].split("_")[0];
            savep(roleid, bb, aa);
        } else {
            // 至少2个被选中
            for (int i = 0; i < funOp.length; i++) {
                String cc = funOp[i].split("_")[0]; // 操作id
                if (i > 0 && bb.equals(funOp[i].split("_")[1])) {
                    aa += "," + cc;
                    if (i == (funOp.length - 1)) {
                        savep(roleid, bb, aa);
                    }
                    ...
                    savep(roleid, bb, aa);
                    aa = funOp[i].split("_")[0]; // 操作ID
                    if (i == (funOp.length - 1)) {
                        bb = funOp[i].split("_")[1]; // 权限id
                        savep(roleid, bb, aa);
                    }
    ...
    private boolean uploadFtpFile(String url, int port, String userName, String passWord, String path, String fileName, InputStream file) {
        ...
        try {
            ...
            ftp.storeFile(new String(fileName.getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1), file);
            file.close();
            ftp.logout();
            success = true;
    ...
    private boolean downFtpFile(String path, OutputStream out) {
        ...
        try {
            ...
            String fileName = path.substring(path.lastIndexOf("/") + 1);
            ftp.changeWorkingDirectory(path.substring(0, path.lastIndexOf("/")));
            ftp.retrieveFile(new String(fileName.getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1), out);
            ftp.logout();
            success = true;
        } catch (SocketException e) {
            logger.info(e.getMessage());
    ...
    public AjaxJson filedeal(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if ("1".equals(upFlag)) {
                ...
                MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
                MultipartFile mf = multipartRequest.getFile("file");// 获取上传文件对象
                String dbpath = FileUtils.fileSave(bizType, mf, null, swfTransform);
                msg = "上传成功";
                j.setMsg(msg);
                j.setObj(dbpath);
                //如果是删除操作
