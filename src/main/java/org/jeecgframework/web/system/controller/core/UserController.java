...
public class UserController extends BaseController {
    ...
    public void menu(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 二级权限菜单
        List<TSFunction> smallActionList = new ArrayList<>();
        if (loginActionList.size() > 0) {
            for (TSFunction function : loginActionList) {
                if (function.getFunctionLevel() == 0) {
                    bigActionList.add(function);
                } else if (function.getFunctionLevel() == 1) {
                    smallActionList.add(function);
                }
            }
        }
        // 菜单栏排序
        Collections.sort(bigActionList, sort);
        Collections.sort(smallActionList, sort);
        String logString = ListtoMenu.getMenu(bigActionList, smallActionList);
    ...
    public AjaxJson delInterfaceUser(@RequestParam(required = true) String userid) {
        ...
        try {
            ...
            if (user != null) {
                String sql = "delete from t_s_interrole_user where user_id = ?0";
...
//			}
            ...
            List<TSDepart[]> resultList = systemService.findHql("from TSDepart d,TSUserOrg uo where d.id=uo.orgId and uo.id=?0", id);
...
//                saveUserOrgList(req, user);
    ...
    public ModelAndView userOrgSelect(HttpServletRequest request) {
        ...
        List<Object[]> orgArrList = systemService.findHql("from TSDepart d,TSUserOrg uo where d.id=uo.tsDepart.id and uo.tsUser.id=?0", new String[]{userId});
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
                ...
                for (TSUser tsUser : tsUsers) {
                    ...
                    if ((roleCodes == null || "".equals(roleCodes)) || (deptCodes == null || "".equals(deptCodes))) {
                        ...
                        if (flag) {
                            ...
                            if (users.size() != 0) {
                                ...
                                String id = user.getId();
                                systemService.executeSql("delete from t_s_role_user where userid = ?0", id);
                                for (String roleCode : roles) {
                                    //根据角色编码得到roleid
                                    List<TSRole> roleList = systemService.findByProperty(TSRole.class, "roleCode", roleCode);
                                ...
                                systemService.executeSql("delete from t_s_user_org where user_id = ?0", id);
