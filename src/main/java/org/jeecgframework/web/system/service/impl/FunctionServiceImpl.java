...
public class FunctionServiceImpl extends CommonServiceImpl implements FunctionService {
    ...
    public AjaxJson delFunction(String functionId) {
        ...
        //删除角色菜单关系
        this.executeSql("delete from t_s_role_function where functionid=?0", functionId);
        //删除权限组和菜单关系
        this.executeSql("delete from t_s_depart_authg_function_rel where auth_id=?0", functionId);
        ...
        try {
            ...
            List<TSOperation> op = this.findHql("from TSOperation where TSFunction.id = ?0", functionId);
