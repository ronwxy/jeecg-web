...
//		log.setTSUser(ResourceUtil.getSessionUser());
    ...
    public TSType getType(String typecode, String typename, TSTypegroup tsTypegroup) {
        ...
        List<TSType> ls = commonDao.findHql("from TSType where typecode = ?0 and typegroupid = ?1", typecode, tsTypegroup.getId());
        TSType actType = null;
    ...
    public void initAllTypeGroups() {
        ...
        for (TSTypegroup tsTypegroup : typeGroups) {
            ...
            List<TSType> types = this.commonDao.findHql("from TSType where TSTypegroup.id = ?0 order by orderNum", tsTypegroup.getId());
            for (TSType t : types) {
                t.setTSType(null);
    ...
    public void refleshTypesCach(TSType type) {
        ...
        List<TSType> tempList = this.commonDao.findHql("from TSType where TSTypegroup.id = ?0 order by orderNum", tsTypegroup.getId());
    ...
    public List<TSOperation> getLoginOperationsByUserId(String userId, String functionId, String orgId) {
        String hql = "FROM TSOperation where functionid = ?0";
