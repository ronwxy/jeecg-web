    ...
    public String trueDel(TSUser user) {
        ...
        if (!user.getStatus().equals(Globals.USER_ADMIN)) {
            if (roleUser.size() > 0) {
                // 删除用户时先删除用户和角色关系表
                delRoleUser(user);
                this.commonDao.executeSql("delete from t_s_user_org where user_id=?0", user.getId()); // 删除 用户-机构 数据
...
//		log.setTSUser(ResourceUtil.getSessionUser());
    ...
    public void saveOrUpdate(TSUser user, String[] orgIds, String[] roleIds) {
        if (StringUtil.isNotEmpty(user.getId())) {
            commonDao.executeSql("delete from t_s_user_org where user_id=?0", user.getId());
    ...
    public Map<String, TSFunction> getLoginUserFunction(String userId) {
        ...
        //查询用户角色对应的授权菜单
        StringBuilder hqlsb1 = new StringBuilder("select distinct f from TSFunction f,TSRoleFunction rf,TSRoleUser ru  ").append("where ru.TSRole.id=rf.TSRole.id and rf.TSFunction.id=f.id and ru.TSUser.id=?0 ");
        //查询用户组织机构授权的菜单
        StringBuilder hqlsb2 = new StringBuilder("select distinct c from TSFunction c,TSRoleFunction rf,TSRoleOrg b,TSUserOrg a ").append("where a.tsDepart.id=b.tsDepart.id and b.tsRole.id=rf.TSRole.id and rf.TSFunction.id=c.id and a.tsUser.id=?0");
            ...
            if (loginFunctionslist.size() > 0) {
                ...
                for (List<TSFunction> list : c) {
                    for (TSFunction function : list) {
                        //如果有子级菜单 则地址设为空
                        if (function.hasSubFunction(functionMap)) {
    ...
    public String getShortcutPrimaryMenu(List<TSFunction> primaryMenu) {
        ...
        for (TSFunction function : primaryMenu) {
            if (function.getFunctionLevel() == 0) {
                ...
                langContext = langContext.trim();

                if ("业务申请".equals(langContext)) {
                    String ss = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>" + langContext + "</span></div>";
                ...
                } else if ("个人办公".equals(langContext)) {
                ...
                } else if ("流程管理".equals(langContext)) {
                ...
                } else if ("Online 开发".equals(langContext)) {
                ...
                } else if ("自定义表单".equals(langContext)) {
                ...
                } else if ("统计报表".equals(langContext)) {
                ...
                } else if ("消息中间件".equals(langContext)) {
                    String ss = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>" + langContext + "</span></div>";
                    ...
                    if (langContext.length() >= 5 && langContext.length() < 7) {
                        s = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>" + langContext + "</span></div>";
                    } else if (langContext.length() < 5) {
                        s = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'>" + langContext + "</div>";
                    } else if (langContext.length() >= 7) {
                        s = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>" + langContext.substring(0, 6) + "</span></div>";
                    }
        ...
        for (TSFunction function : primaryMenu) {
            if (menuString.contains(function.getFunctionName())) {
                if (function.getFunctionLevel() == 1) {
                    ...
                    String langKey = function.getFunctionName();
                    String langContext = mutiLangService.getLang(langKey);
                    if ("申请".equals(langKey)) {
                        langContext = "申请";
                        String s = "";
                        s = "<div style='width:67px;position: absolute;top:47px;text-align:center;color:#000000;font-size:12px;'>" + langContext + "</div>";
                        ...
                        String s = "";
                        if (langContext.length() >= 5 && langContext.length() < 7) {
                            s = "<div style='width:67px;position: absolute;top:40px;text-align:center;color:#000000;font-size:12px;'><span style='letter-spacing:-1px;'>" + langContext + "</span></div>";
                        } else if (langContext.length() < 5) {
                            s = "<div style='width:67px;position: absolute;top:40px;text-align:center;color:#000000;font-size:12px;'>" + langContext + "</div>";
                        } else if (langContext.length() >= 7) {
                            s = "<div style='width:67px;position: absolute;top:40px;text-align:center;color:#000000;font-size:12px;'><span style='letter-spacing:-1px;'>" + langContext.substring(0, 6) + "</span></div>";
                        }
