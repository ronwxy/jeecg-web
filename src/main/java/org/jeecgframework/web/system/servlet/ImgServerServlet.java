...
//      String path = this.getServletContext().getRealPath(imgurl);
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (url.indexOf(ONLINE_TEMPLATE_BASE_PATH) != -1) {
            this.showPicOnlineTemplate(url, response);
        } else if (url.indexOf(WEB_UPLOAD_BASE_PATH) != -1) {
            this.showPicWebUpload(url, request, response);
        }
    }
     ...
     * @throws IOException
     */
    private void showPicWebUpload(String url, HttpServletRequest request, HttpServletResponse response) throws IOException {
        String imgPath = url.substring(url.indexOf(WEB_UPLOAD_BASE_PATH)).replace(WEB_UPLOAD_BASE_PATH, "");
        //URL特殊字符转义
        ...
        if ("true".equals(down)) {
            ...
            if (FileUtils.isPicture(imgPath)) {
                response.setContentType("image/jpeg;charset=utf-8");
            } else if (imgPath.matches("^.*\\.(mp4)|(mov)$")) {
     ...
     * @throws IOException
     */
    private void showPicOnlineTemplate(String url, HttpServletResponse response) throws IOException {
        String imgPath = url.substring(url.indexOf(ONLINE_TEMPLATE_BASE_PATH)).replace(ONLINE_TEMPLATE_BASE_PATH, "");
        //URL特殊字符转义
        imgPath = URLDecoder.decode(imgPath, "utf-8");
        response.setContentType("image/jpeg;charset=utf-8");
        InputStream inputStream = null;
