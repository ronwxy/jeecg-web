    ...
    public MsgDeliver(byte[] data) {
        if (data.length > 8 + 8 + 21 + 10 + 1 + 1 + 1 + 32 + 1 + 1 + 1 + 20) {// +Msg_length+
            ...
            try {
                ...
                byte[] serviceIdByte = new byte[10];
                dins.read(serviceIdByte);
                this.serviceId = new String(serviceIdByte);// 10 业务标识 String
                this.tPPid = dins.readByte();
                this.tPUdhi = dins.readByte();
                this.msgFmt = dins.readByte();
                fmtStr = this.msgFmt == 8 ? "utf-8" : "gb2312";
                byte[] srcTerminalIdByte = new byte[32];
                dins.read(srcTerminalIdByte);
                this.srcTerminalId = new String(srcTerminalIdByte);// 源终端MSISDN号码
                ...
                byte[] msgContentByte = new byte[msgLength];
                dins.read(msgContentByte);
                if (registeredDelivery == 1) {
                    this.msgContent = new String(msgContentByte, fmtStr);// 消息长度
                    if (msgLength == 8 + 7 + 10 + 10 + 21 + 4) {
                        ...
                        dinsC.read(submitTimeByte);
                        this.submitTime = new String(submitTimeByte, fmtStr);
                        byte[] doneTimeByte = new byte[7];
                        dinsC.read(doneTimeByte);
                        this.doneTime = new String(doneTimeByte, fmtStr);
                        byte[] destTerminalIdByte = new byte[21];
                        dinsC.read(destTerminalIdByte);
                        this.destTerminalId = new String(destTerminalIdByte,
                                fmtStr);
