...
<div class="easyui-layout" fit="true">
  ...
  title="${ftl_description}" actionUrl="${entityName?uncap_first}Controller.do?datagrid" idField="id"  queryMode="group">
   ...
   <#list columns as po>
    <#if po.isShowList == 'N'>
    <t:dgCol title="${po.content}"  field="${po.fieldName}"  hidden="true" />
    <#elseif po.showType == 'date'>
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if> <#if po.dictField??><#if  po.dictField != "">dictionary="${po.dictField}"</#if></#if> formatter="yyyy-MM-dd" extendParams="editor:'datebox'" width="100"/>
    <#elseif po.showType == 'textarea'>
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if> <#if po.dictField??><#if  po.dictField != "">dictionary="${po.dictField}"</#if></#if> extendParams="editor:'textarea'" width="100"/>
    <#elseif po.showType == 'checkbox' >
    <#-- update--begin--author:zhoujf date:20180330 for:TASK #2592 【代码生成器】单表档案行编辑风格表单，代码生成，点击页面编辑所有的数据不可操作 -->
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if> <#if po.dictTable?if_exists?html!="">dictionary="${po.dictTable},${po.dictField},${po.dictText}" <#else><#if po.dictField??><#if  po.dictField != "">dictionary="${po.dictField}"</#if></#if></#if> extendParams="editor:'combobox'" width="100"/>
    ...
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if> <#if po.dictField??><#if  po.dictField != "">dictionary="${po.dictField}"</#if></#if> formatter="yyyy-MM-dd hh:mm:ss" extendParams="editor:'datebox'" width="100"/>
    <#elseif po.showType == 'list'> 
    <#-- update--begin--author:zhoujf date:20180330 for:TASK #2592 【代码生成器】单表档案行编辑风格表单，代码生成，点击页面编辑所有的数据不可操作 -->
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if> <#if po.dictTable?if_exists?html!="">dictionary="${po.dictTable},${po.dictField},${po.dictText}" <#else><#if po.dictField??><#if  po.dictField != "">dictionary="${po.dictField}"</#if></#if></#if> extendParams="editor:'combobox'" width="100"/>
    <#-- update--end--author:zhoujf date:20180330 for:TASK #2592 【代码生成器】单表档案行编辑风格表单，代码生成，点击页面编辑所有的数据不可操作 -->
    <#elseif po.type == 'int'> 
    <t:dgCol title="${po.content}"  field="${po.fieldName}" queryMode="${po.queryMode}" <#if  po.isQuery == 'Y'>query="true"</#if> <#if po.dictField??><#if  po.dictField != "">dictionary="${po.dictField}"</#if></#if> extendParams="editor:'numberbox'" width="100"/>
    ...
    <t:dgCol title="${po.content}"  field="${po.fieldName}" extendParams="editor:'text'" image="true" imageSize="50,50" width="100"/>
	<#elseif po.showType?index_of("file") != -1>
    <t:dgCol title="${po.content}"  field="${po.fieldName}" extendParams="editor:'text'" downloadName="附件下载" width="100"/>
   ...
   <#--//update-end--Author:zhangjiaqiang  Date:20160925 for：TASK #1344 [链接图标] online功能测试的按钮链接图标修改 -->
    <t:dgToolBar operationCode="add" title="录入" icon="icon-add"  funname="addRow"/>
    <t:dgToolBar operationCode="edit" title="编辑" icon="icon-edit"  funname="editRow"/>
    <t:dgToolBar operationCode="save" title="保存" icon="icon-save" url="${entityName?uncap_first}Controller.do?saveRows" funname="saveData"/>
    <t:dgToolBar operationCode="undo" title="取消编辑" icon="icon-undo" funname="reject"/>
    <t:dgToolBar title="批量删除"  icon="icon-remove" url="${entityName?uncap_first}Controller.do?doBatchDel" funname="deleteALLSelect"/>
  </t:datagrid>
  </div>
 </div>
