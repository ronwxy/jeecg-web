...
<t:datagrid name="jeecgDemoList2" title="高级查询示例"  actionUrl="jeecgListDemoController.do?datagrid"
	...
	<t:dgCol title="用户名" field="name" query="true" />
	<t:dgCol title="个人介绍"  field="content" query="true"/>
	<t:dgCol title="办公电话" field="phone" query="true"/>
	<t:dgCol title="创建日期" field="createDate" formatter="yyyy-MM-dd" query="true" queryMode="group"/>
	<t:dgCol title="邮箱" field="email" query="true"/>
	<t:dgCol title="年龄" sortable="true" field="age" query="true"/>
	<t:dgCol title="工资" field="salary" query="true"/>

	<t:dgCol title="生日" field="birthday" formatter="yyyy/MM/dd" query="true"/>
	<t:dgToolBar  title="录入" icon="icon-edit" url="jeecgDemoController.do?addorupdate" funname="add"/>
	<t:dgToolBar  title="编辑" icon="icon-edit" url="jeecgDemoController.do?addorupdate" funname="update"/>
	<t:dgToolBar  title="高级查询" icon="icon-search" funname="queryBuilder" />
</t:datagrid></div>
</div>
