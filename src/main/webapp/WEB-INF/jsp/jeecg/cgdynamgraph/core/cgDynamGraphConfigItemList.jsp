	...
	<tbody id="add_cgDynamGraphConfigItem_table">
		<c:if test="${fn:length(cgDynamGraphConfigItemList)  <= 0 }">
			<tr>
				...
				<td align="left"><t:dictSelect field="cgDynamGraphConfigItemList[0].fieldType"  extendJson="{style:'width:80px'}" type="list" typeGroupCode="fieldtype" defaultVal="${cgDynamGraphConfigItemPage.fieldType}" hasLabel="false" title="common.text.type"/></td>
				...
				<td align="left"><t:dictSelect field="cgDynamGraphConfigItemList[0].SMode" type="list"  extendJson="{style:'width:90px'}" typeGroupCode="searchmode" defaultVal="${cgDynamGraphConfigItemPage.sMode}" hasLabel="false" title="common.query.module"/></td>
				...
				<td align="left"><t:dictSelect field="cgDynamGraphConfigItemList[0].SFlag"  extendJson="{style:'width:90px'}" divClass="STYLE_LEG" type="list" typeGroupCode="yesorno" defaultVal="${cgDynamGraphConfigItemPage.sFlag}" hasLabel="false" title="common.isquery"/></td>
			</tr>
		</c:if>
		<c:if test="${fn:length(cgDynamGraphConfigItemList)  > 0 }">
			<c:forEach items="${cgDynamGraphConfigItemList}" var="poVal" varStatus="stuts">
				<tr>
					...
					<td align="left"><t:dictSelect field="cgDynamGraphConfigItemList[${stuts.index }].fieldType" type="list" extendJson="{style:'width:80px'}" typeGroupCode="fieldtype" defaultVal="${poVal.fieldType}" hasLabel="false" title="common.text.type"/>
					</td>
					...
					<td align="left"><t:dictSelect field="cgDynamGraphConfigItemList[${stuts.index }].SMode"  extendJson="{style:'width:90px'}" type="list" typeGroupCode="searchmode" defaultVal="${poVal.SMode}" hasLabel="false" title="common.query.module"/></td>
					...
					<td align="left"><t:dictSelect field="cgDynamGraphConfigItemList[${stuts.index }].SFlag" extendJson="{style:'width:90px'}" type="list" typeGroupCode="yesorno" defaultVal="${poVal.SFlag}" hasLabel="false" title="common.isquery"/></td>
				</tr>
			</c:forEach>
		</c:if>
	</tbody>
