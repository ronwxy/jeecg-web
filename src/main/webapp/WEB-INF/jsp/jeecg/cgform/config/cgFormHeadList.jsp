...
<t:base type="jquery,easyui,tools,autocomplete"/>
...
<div region="center" style="padding:0px;border:0px">
    <t:datagrid sortName="createDate" sortOrder="desc" name="tablePropertyList" title="smart.form.config"
                fitColumns="true" actionUrl="cgFormHeadController.do?datagrid" idField="id" fit="true"
                queryMode="group" checkbox="true" btnCls="bootstrap" pageSize="20">
	<t:dgCol title="common.id" field="id" hidden="true"/>
	<t:dgCol title="hasPeizhi" field="hasPeizhi" hidden="true"/>
	<t:dgCol title="table.type" field="jformType" replace="single.table_1,master.table_2,slave.table_3" query="true" width="60"/>
	<t:dgCol title="table.name" field="tableName" query="true" autocomplete="true" width="150"/>
	<t:dgCol title="form.category" field="jformCategory" dictionary="bdfl" width="80"/>
	<t:dgCol title="table.description" field="content" width="150"/>
	<t:dgCol title="common.version" field="jformVersion" width="50"/>
	<t:dgCol title="is.tree" field="isTree" hidden="true" replace="common.yes_Y,common.no_N" width="60"/>
	<t:dgCol title="is.page" field="isPagination" hidden="true" replace="common.yes_Y,common.no_N" width="60"/>
	<t:dgCol title="sync.db" field="isDbSynch" replace="has.sync_Y,have.nosync_N" style="background:red;_N" query="true" width="80"/>
	<t:dgCol title="show.checkbox" field="isCheckbox" hidden="true" replace="common.yes_Y,common.no_N" width="60"/>
	<t:dgCol title="common.query.module" field="querymode" hidden="true" width="100"/>
	<t:dgCol title="common.createby" field="createBy" hidden="true" />
	<t:dgCol title="common.createtime" field="createDate" formatter="yyyy/MM/dd" hidden="true"/>
	<t:dgCol title="common.updateby" field="updateBy" hidden="true"/>
	<t:dgCol title="common.updatetime" field="updateDate" formatter="yyyy/MM/dd" hidden="true"/>
	<t:dgCol title="common.operation" field="opt" width="500"/>
        <t:dgFunOpt funname="delCgForm(id,tableName)" title="common.delete" urlclass="ace_button"
                    urlStyle="background-color:#ec4758;" urlfont="fa-trash-o"/>
        <t:dgFunOpt funname="remCgForm(id)" title="common.remove" urlclass="ace_button"
                    urlStyle="background-color:#FFA500;"
                    urlfont="fa-remove"/>
	<%-- 
	<t:dgFunOpt funname="importFields(id,content)" title="导入字段" urlclass="ace_button"  urlfont="fa-download"/>
	--%>
	<t:dgFunOpt exp="isDbSynch#eq#N"  title="sync.db" funname="doDbsynch(id,content)" urlclass="ace_button"  urlfont="fa-database"/>
        <t:dgFunOpt exp="isDbSynch#eq#Y&&jformType#ne#3" funname="addbytab(id,content)" title="form.template"
                    urlclass="ace_button" urlfont="fa-cog"/>
        <t:dgFunOpt exp="isDbSynch#eq#Y&&jformType#ne#3" funname="addlisttab(tableName,content)" title="function.test"
                    urlStyle="background-color:#18a689;" urlclass="ace_button" urlfont="fa-gavel"/>
        <t:dgFunOpt exp="isDbSynch#eq#Y&&jformType#ne#3" funname="popMenuLink(tableName,content)" title="config.place"
                    urlStyle="background-color:#1a7bb9;" urlclass="ace_button" urlfont="fa-cog"/>
        <t:dgFunOpt funname="copyOnline(id)" title="复制表单" operationCode="copyOnlineTable" urlclass="ace_button"
                    urlfont="fa-copy"/>
        <t:dgFunOpt exp="hasPeizhi#ne#0" funname="propertyTable(id)" title="配置表" urlclass="ace_button"
                    urlfont="fa-cog"/>
	<t:dgToolBar title="create.form" icon="fa fa-plus" width="100%" height="100%" url="cgFormHeadController.do?addorupdate" funname="addForm"/>
	<t:dgToolBar title="edit.form" icon="fa fa-edit" width="100%" height="100%" url="cgFormHeadController.do?addorupdate" funname="updateForm"/>
	<t:dgToolBar title="custom.button" icon="fa fa-bars" url="cgformButtonController.do?cgformButton" funname="cgFormButton"/>
	<t:dgToolBar title="js.enhance" icon="fa fa-strikethrough" url="cgformEnhanceJsController.do?addorupdate" funname="enhanceJs"/>
	<t:dgToolBar title="sql.enhance" icon="fa fa-filter" url="cgformButtonSqlController.do?addorupdate" operationCode="sql_enhance" funname="cgFormButtonSql"/>
	<t:dgToolBar title="java.enhance" icon="fa fa-wrench" url="cgformEnhanceJavaController.do?addorupdate" funname="javaEnhance"/>
	<t:dgToolBar title="form.export" icon="fa fa-download" url="cgformSqlController.do?doMigrateOut" funname="doMigrateOut"/>
	<t:dgToolBar title="form.import" operationCode="form_import" icon="fa fa-upload" url="cgformSqlController.do?inSqlFile" funname="toCgformMigrate"/>
	<t:dgToolBar title="code.generate" operationCode="code_generate" icon="fa fa-spinner" url="generateController.do?gogenerate" funname="generate"/>
	<t:dgToolBar title="form.generate" icon="fa fa-database" url="cgformTransController.do?trans" operationCode="db_generate_form" funname="addToData"/>
</t:datagrid>
</div>
</div>
