		...
		<tr>
			...
			<td class="value" colspan="3"><input type="checkbox" value="1" name="actionFlag" id="actionFlag" checked="checked">Action</input> <input type="checkbox" value="1" name="jspFlag" id="jspFlag"
				...
				<div style="display:inline-block">
					...
					<input type="radio" name="supportRestful" checked="checked" value="1"/>是
					<input type="radio" name="supportRestful" value="0"/>否
					<span>】</span>
				</div>
			</td>
		</tr>
		...
		<tr id="jspMode_tr">
			...
			<select id="jspMode" name="jspMode" style="width: 300px" datatype="*">
		     		<c:forEach items="${jspModeList }" var="style">
						<option value="${style.code }"  <c:if test="${style.code eq 'bootstraptableUI.single'}"> selected='selected'</c:if>>${style.desc }</option>
			     	</c:forEach>
		     </select>
			<span class="Validform_checktip"></span></td>
		</tr>
