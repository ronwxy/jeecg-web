...
<div id="main_depart_list" class="easyui-layout" fit="true">
    <div region="center" style="padding:0px;border:0px">
        <t:datagrid name="departList" title="common.department.list" fitColumns="true"
                    actionUrl="departController.do?departgrid" treegrid="true" idField="departid" pagination="false">
            <t:dgCol title="common.id" field="id" treefield="id" hidden="true"/>
            <t:dgCol title="common.id" field="parentId" treefield="parentId" hidden="true"/>
            <t:dgCol title="common.department.name" field="departname" treefield="text" width="120"/>
            <t:dgCol title="position.desc" field="description" treefield="src" width="70"/>
            <t:dgCol title="common.org.code" field="orgCode" treefield="fieldMap.orgCode" width="50"/>
            <t:dgCol title="common.org.type" field="orgType" dictionary="orgtype" treefield="fieldMap.orgType"
                     width="60"/>
            <t:dgCol title="common.mobile" field="mobile" treefield="fieldMap.mobile" width="60"/>
            <t:dgCol title="common.fax" field="fax" treefield="fieldMap.fax" width="60"/>
            <t:dgCol title="common.address" field="address" treefield="fieldMap.address" width="100"/>
            <t:dgCol title="common.operation" field="opt" width="200"/>
            ...
            <t:dgFunOpt funname="delDepart(id)" title="common.delete" urlclass="ace_button"
                        urlStyle="background-color:#ec4758;" urlfont="fa-trash-o"/>
            <t:dgFunOpt funname="queryUsersByDepart(id)" title="view.member" urlclass="ace_button" urlfont="fa-user"/>
            <t:dgFunOpt funname="setRoleByDepart(id,text)" title="role.set" urlclass="ace_button" urlfont="fa-cog"
                        urlStyle="background-color:#1a7bb9;"/>
            <t:dgFunOpt funname="setExtendByDepart(id,text)" title="扩展设置" urlclass="ace_button"
                        urlStyle="background-color:#18a689;" urlfont="fa-cog"/>
        </t:datagrid>
        <div id="departListtb" style="padding: 3px; height: 25px">
            <div style="float: left;">
                <a href="#" class="easyui-linkbutton" plain="true" icon="icon-add" onclick="addOrg()"><t:mutiLang
                        langKey="common.add.param" langArg="common.department"/></a>
                <a href="#" class="easyui-linkbutton" plain="true" icon="icon-edit"
                   onclick="update('<t:mutiLang langKey="common.edit.param"
                                                langArg="common.department"/>','departController.do?update','departList','680px','450px')"><t:mutiLang
                        langKey="common.edit.param" langArg="common.department"/></a>
                ...
                -->
                <a href="#" class="easyui-linkbutton" plain="true" icon="icon-putout" onclick="ExportXls()"><t:mutiLang
                        langKey="excelOutput" langArg="common.department"/></a>
                <a href="#" class="easyui-linkbutton" plain="true" icon="icon-putout"
                   onclick="ExportXlsByT()"><t:mutiLang langKey="templateDownload" langArg="common.department"/></a>
                <a href="#" class="easyui-linkbutton" plain="true" icon="icon-put"
                   onclick="ImportDepartXls()"><t:mutiLang langKey="excelDepartImport" langArg="common.department"/></a>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function setExtendByDepart(departId, text) {
        var title = text + '扩展设置';
        $('#main_depart_list').layout('panel', 'east').panel('setTitle', title);
        $('#main_depart_list').layout('panel', 'east').panel('resize', {width: 560});
        if (li_east == 0) {
        ...
        var url = {
            <%--title :"test",--%>
            href: "cnbotsDepartExtendController.do?extend&departId=" + departId
        };
        $('#userListpanel').panel(url);
    }
