...
<t:datagrid name="fList" title="文件下载" actionUrl="jeecgFormDemoController.do?documentList&typecode=files" idField="id" fit="true" sortName="createdate" sortOrder="desc">
	...
	<t:dgCol title="标题" field="documentTitle" width="20" query="true"/>
	<t:dgCol title="创建时间" field="createdate" formatter="yyyy-MM-dd hh:mm:ss"  width="20"/>
	<t:dgCol title="类名" field="subclassname" hidden="true"/>
	...
	<t:dgToolBar title="文件录入" icon="icon-add" funname="add" url="jeecgFormDemoController.do?addFiles"/>
	<t:dgToolBar title="通用文件录入" icon="icon-add" funname="commmonUpload"/>
	<t:dgToolBar title="编辑" icon="icon-edit" funname="update" url="jeecgFormDemoController.do?editFiles"/>
</t:datagrid>
