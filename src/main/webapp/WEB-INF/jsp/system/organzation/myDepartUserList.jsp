...
<div id="divUserList" class="easyui-layout" style="width:100%;">
	...
	<t:datagrid name="departUserList" title="common.operation"
		...
		<t:dgCol title="common.username" sortable="false" field="userName" query="true" width="50"/>
		<t:dgCol title="common.real.name" field="realName" query="true" width="50"/>
		<t:dgCol title="common.department" sortable="false" field="userOrgList.tsDepart.departname" query="false" width="60"/>
		<t:dgCol title="common.status" sortable="true" width="20" field="status" replace="common.active_1,common.inactive_0,super.admin_-1"/>
		<t:dgCol title="common.operation" field="opt" width="130"/>
        ...
        <t:dgFunOpt funname="setUsersCompanyPosition(id)" title="分配职务" urlclass="ace_button" urlfont="fa-user"/>
		<t:dgToolBar title="common.add.param" width="800" height="500" langArg="common.user" icon="icon-add" url="userController.do?addorupdateMyOrgUser&departid=${departid}" funname="add"/>
		<t:dgToolBar title="common.edit.param" width="800" height="500"  langArg="common.user" icon="icon-edit" url="userController.do?addorupdateMyOrgUser&departid=${departid}" funname="update"/>
		<t:dgToolBar title="添加已有用户" icon="icon-add" url="organzationController.do?goAddMyOrgUserToOrg&orgId=${departid}" funname="add" width="650"/>
        <t:dgFunOpt funname="openAuthorizeSet(id)" title="分配部门角色" urlclass="ace_button" urlfont="fa-cog"
                    urlStyle="background-color:#18a689;"/>
	</t:datagrid>
 </div>
</div>
